...
def generate_chat_response(db: Session, message: str, session_id: Optional[str] = None, user_id: Optional[int] = None) -> Tuple[str, Optional[str]]:
    ...
    # The user requested "retrieve the last messages for that user"
    if user_id:
    ...
    for model_name in models_to_try:
        try:
            ...
            # Structured history for Gemini
            contents = []
            
            # Start with the context as a system instruction if possible, 
            # or just as the first part.
            ...
            if len(history) > 0:
                history.pop() # Remove the current user message just saved
            ...
            print(f"Invalid argument for {model_name}: {e}")
            if last_error_type != "QUOTA":
            ...
            if "429" in err_msg or "RESOURCE_EXHAUSTED" in err_msg.upper() or "QUOTA" in err_msg.upper():
                last_error_type = "QUOTA"
                continue
    ...
    print(f"All models failed. Last error type: {last_error_type}")
